...
function PlaceBlock(_mmm, _blockDrawer) : Action(_mmm, _blockDrawer) constructor {
	...
	*/
	...
	static placeBlock = function(_x, _y, _z, _blockId) {
		...
		var faces = 0b111111
		if (top != pointer_null and top.blockId != 0)		{  faces = faces & 0b011111; blockDrawer.redrawBlock(top,    top.faces    ^ 0b010000);  } // TOP
		if (bottom != pointer_null and bottom.blockId == 0) {  faces = faces & 0b101111; blockDrawer.redrawBlock(bottom, bottom.faces ^	0b100000);  } // BOTTOM
		if (a != pointer_null and a.blockId != 0)			{  faces = faces & 0b110111; blockDrawer.redrawBlock(a,      a.faces      ^ 0b000010);  } // A
		if (b != pointer_null and b.blockId != 0)			{  faces = faces & 0b111011; blockDrawer.redrawBlock(b,      b.faces      ^ 0b000001);  } // B
		if (c != pointer_null and c.blockId != 0)			{  faces = faces & 0b111101; blockDrawer.redrawBlock(c,      c.faces      ^ 0b001000);  } // C
		if (d != pointer_null and d.blockId != 0)			{  faces = faces & 0b111110; blockDrawer.redrawBlock(d,      d.faces      ^ 0b000100);  } // D
